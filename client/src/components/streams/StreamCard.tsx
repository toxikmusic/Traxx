...
export default function StreamCard({ stream }: StreamCardProps) {
  ...
  return (
    <div className="bg-card border themed-border rounded-lg overflow-hidden group">
      <Link href={`/streams/${stream.id}`}>
        <div className="relative cursor-pointer">
          ...
          />
          {stream.isLive && (
            <div className="absolute top-2 left-2 bg-primary stream-live-badge text-primary-foreground text-xs font-semibold px-2 py-0.5 rounded-md flex items-center">
              <span className="inline-block w-2 h-2 bg-white rounded-full animate-pulse mr-1.5"></span>
              LIVE
            </div>
          )}
          <div className="absolute bottom-2 left-2 bg-black/70 text-xs font-medium px-2 py-0.5 rounded-md">
        ...
        <div className="flex justify-between">
          <div className="flex space-x-2">
            <Link href={`/profile/${stream.userId}`}>
              <Avatar className="w-10 h-10 cursor-pointer avatar-highlight">
                {/* In a real app, we'd have the user data here */}
                <AvatarImage src={`https://source.unsplash.com/random/${stream.userId}`} />
                <AvatarFallback className="bg-primary/10 text-primary">{stream.title[0]}</AvatarFallback>
              </Avatar>
            </Link>
            
            <div>
              <Link href={`/streams/${stream.id}`}>
                <h3 className="font-medium cursor-pointer hover:text-primary transition-colors">{stream.title}</h3>
              </Link>
              
              <Link href={`/profile/${stream.userId}`}>
                <p className="text-sm text-muted-foreground cursor-pointer hover:text-primary transition-colors">{stream.userId}</p>
              </Link>
              
              <div className="flex items-center space-x-2 mt-1">
                {stream.tags && stream.tags.slice(0, 2).map((tag, idx) => (
                  <Badge key={idx} variant="outline" className="text-xs px-2 py-0.5 border-primary/30 hover:bg-primary/5">
                    {tag}
                  </Badge>
                ))}
              </div>
